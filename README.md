...
- Grab a moving cursor to solo it: the other cursors freeze and fade to silence. Drag forward or backward along connected edges, passing through shared nodes to change edges. Release to resume all unfinished graphs from their current positions, with no catch-up. Automatic playback always follows arrows; manual movement does not consume branch counters. Space or an interrupted grab stops the performance.
